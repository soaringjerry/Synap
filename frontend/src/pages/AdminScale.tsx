              ...
              {showAdvancedConsent && (
                <div className="tile" style={{padding:10, marginTop:8}}>
                  ...
                  <div className="muted" style={{marginTop:8}}>
                    {t('consent.advanced.key_help_export') || 'Key is used in exports as CSV column consent.<key>; autogenerated from label. You can edit if needed.'}
                  </div>
                      ...
                      <div style={{display:'grid', gridTemplateColumns:'1.4fr 1.4fr 0.8fr 0.8fr', gap:8, alignItems:'end'}}>
                        <div>
                          ...
                          {newConsent.showKey && (
                            <div style={{marginTop:6}}>
                              <div className="label">{t('consent.advanced.key')||'Key'}</div>
                              <input className="input" value={newConsent.key} onChange={e=> setNewConsent(v=> ({...v, key: sanitizeKey(e.target.value)}))} placeholder="e.g., recording" />
                              <div className="muted" style={{marginTop:4}}>{t('consent.advanced.key_hint')||'Use lowercase letters/numbers/_/-. Example: recording'}</div>
                            </div>
                          )}
                        ...
                        <div className="cta-row" style={{gap:8}}>
                          <button className="btn" onClick={()=>{
                            let k = newConsent.key.trim()
                            if (!k) k = sanitizeKey(newConsent.en || newConsent.zh || `custom_${consentOptions.length+1}`)
                            if (!k) return
                            if (consentOptions.some(o=> o.key===k)) { alert(t('consent.advanced.duplicate_key')||'Duplicate key'); return }
                            const next = [...consentOptions, { key: k, required: newConsent.required, en: newConsent.en || undefined, zh: newConsent.zh || undefined }]
                            setConsentOptions(next)
                            saveConsentWith(next)
                            setNewConsent({ en:'', zh:'', required:false, key:'', showKey:false, open:false })
                          }}>{t('create')||'Create'}</button>
                          <button className="btn btn-ghost" onClick={()=> setNewConsent({ en:'', zh:'', required:false, key:'', showKey:false, open:false })}>{t('cancel')||'Cancel'}</button>
                        </div>
                      </div>
                    )}
                  </div>
